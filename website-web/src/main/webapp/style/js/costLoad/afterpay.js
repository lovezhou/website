...
function stateReadonly()
{
	...
	$("[id^='tr_']").each(function(){
		...
		//更新提交控件名称
		$("#txt_costLoadAfterNow_"+loadId).attr("name","costLoadAfterGoings["+lineNo+"].costLoadAfterNow");
		$("#hid_loadId_"+loadId).attr("name","costLoadAfterGoings["+lineNo+"].loadId");
		//主体页面序号
...
function getSum()
{
	...
	var costLoadSum = 0;
	var costLoadAfterSum = 0;
	var costLoadAfterTrueSum = 0;
	var costLoadAfterNow = 0;
	//计算合计
	...
	$("[id^='span_costLoadAfter_']").each(function (){
		costLoadAfterSum+=Number($(this).text());
	});
	$("[id^='span_costLoadAfterTrue_']").each(function (){
		costLoadAfterTrueSum+=Number($(this).text());
	});
	$("[id^='txt_costLoadAfterNow_']").each(function (){
		costLoadAfterNow+=Number($(this).val());
	});
	...
	$("#span_sumCostLoadAfter").text(costLoadAfterSum);
	$("#span_sumCostLoadAfterTrue").text(costLoadAfterTrueSum);
	$("#span_sumCostLoadAfterNow").text(costLoadAfterNow);
	
	$("#hid_sum").val(0-Number(costLoadAfterNow));
}