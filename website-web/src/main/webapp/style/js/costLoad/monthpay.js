...
function stateReadonly()
{
	...
	$("[id^='tr_']").each(function(){
		...
		//更新提交控件名称
		$("#txt_costLoadMonthNow_"+loadId).attr("name","costLoadMonthGoings["+lineNo+"].costLoadMonthNow");
		$("#hid_loadId_"+loadId).attr("name","costLoadMonthGoings["+lineNo+"].loadId");
		//主体页面序号
...
function getSum()
{
	...
	var costLoadSum = 0;
	var costLoadMonthSum = 0;
	var costLoadMonthTrueSum = 0;
	var costLoadMonthNow = 0;
	//计算合计
	...
	$("[id^='span_costLoadMonth_']").each(function (){
		costLoadMonthSum+=Number($(this).text());
	});
	$("[id^='span_costLoadMonthTrue_']").each(function (){
		costLoadMonthTrueSum+=Number($(this).text());
	});
	$("[id^='txt_costLoadMonthNow_']").each(function (){
		costLoadMonthNow+=Number($(this).val());
	});
	...
	$("#span_sumCostLoadMonth").text(costLoadMonthSum);
	$("#span_sumCostLoadMonthTrue").text(costLoadMonthTrueSum);
	$("#span_sumCostLoadMonthNow").text(costLoadMonthNow);
	
	$("#hid_sum").val(0-Number(costLoadMonthNow));
}