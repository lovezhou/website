...
function stateReadonly()
{
	...
	$("[id^='tr_']").each(function(){
		...
		$("#txt_costPayMessageCurNow_"+carryId).attr("name","nowlist["+lineNo+"].costPayMessageCurNow");
		$("#txt_costPayReturnCurNow_"+carryId).attr("name","nowlist["+lineNo+"].costPayReturnCurNow");
...
function getSum()
{
	...
	var ReturnCurSum = 0;
	var ReturnCurTrueSum = 0;
	var ReturnCurNowSum = 0;
	$("[id^='span_costCarryReturnCur_']").each(function (){
		ReturnCurSum+=Number($(this).text());
	});
	$("[id^='span_costPayReturnCurTrue_']").each(function (){
		ReturnCurTrueSum+=Number($(this).text());
	});
	$("[id^='txt_costPayReturnCurNow_']").each(function (){
		ReturnCurNowSum+=Number($(this).val());
	});
	$("#span_sumReturnCur").text(ReturnCurSum);
	$("#span_sumReturnTrue").text(ReturnCurTrueSum);
	$("#span_sumReturnNow").text(ReturnCurNowSum);
