...
$(function() {
	...
	//弹出窗口数量改变事件
	jessrun.check_top_Number = function(cargoId){
	...
	$("[id^='chk_top_']").change(function(){
		...
		var nowValue =$("#txt_top_loadCargoNumber_"+cargoId).val();
		parent.$("#txt_loadCargoNumber_"+cargoId).val(nowValue);
		var costLoadCargo =$("#txt_top_costLoadCargoAfter_"+cargoId).val();
		parent.$("#txt_costLoadCargoAfter_"+cargoId).val(costLoadCargo);
		//计算主体页面的数量合计
		parent.getSum();
		//判断主体页面的站点是否可以修改
...
{
	checkMoney($(obj));
	var cargoId = $(obj).attr("cargoID");
	var nowValue =$("#txt_top_costLoadCargoAfter_"+cargoId).val();
	parent.$("#txt_costLoadCargoAfter_"+cargoId).val(nowValue);
	parent.getSum();
}
