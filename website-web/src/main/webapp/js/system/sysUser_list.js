...
function queryParams(){
	...
	var loginAccount =  EasyUI.getValue("txt_loginAccount");
	var accountId =  EasyUI.getValue("txt_accountId");
	var params = {
			name:name,
...
function deleteRow(id){
    if (id){
        $.messager.confirm('确认','确实删除这条数据?',function(r){
            if (r){
                $.post('sysUser/deleteById.do',{id:id},function(result){
                    if (result.success){
                    	search();
                    } else {
...
function deleteRows(){
	...
	if(rows.length<1){
		...
		$.messager.confirm('确认','确实删除['+rows.length+']条数据?',function(r){
	            if (r){
	                $.post('sysUser/deleteByIds.do',{ids:ids},function(result){
	                    if (result.success){
	                    	search();
	                    } else {
	                        $.messager.show({    // show error message
