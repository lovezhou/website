	...
	<head>
		...
		<script type="text/javascript">
			 ...
			 if (str){
			  ...
			  for (var i=0; i<str.length; i++)
			  {
			   ...
			   if (isEnKong1(str.charAt(i))) {
			    ...
			    pStr += pinyin(str.charAt(i)).substring(0,1).toUpperCase();
			   }
			  }
			  return Trim(pStr);
			 }
			...
			$(document).ready(function(){
				...
				$("#menu_top").load("<%=basePath%>/top.do",function(){
					$("#topmenu ul li a").each(function(){
						var menuElement = $(this);
						if(menuElement.length > 0)
						{
							var menu = {};
							var menuName = menuElement.find("span").html();
							var functionString = menuElement.attr("onClick").toString();
							var changString = functionString.substring(functionString.indexOf("change("), functionString.indexOf(");") + 1);
							menu.name = Trim(menuName);
							menu.href = changString.split(",")[1].replace(/'/g,"");
							menu.token = "ROLE_" + toPinyin(menuName);
							menuArray.push(menu);
						}
					});	
					
					//添加 menuLeft
					menuLeft(0, menuArray);
				});
			});
			
			
			
			var menuLeft = function(idx, menuArray){
				if(idx >= menuArray.length)
				{
					return;
				}
				$("#menu_left").load("<%= basePath%>" + menuArray[idx++].href, function(){
					...
					{
						var token_2="";
						$("#masterdiv").children().each(function(index){
						var name = Trim($(this).children().eq(0).html()); 
						var token = menuArray[idx-1].token + "_" + toPinyin(name);
						token = removeSymbols(token);
					    $(this).children().eq(1).find("a").each(function(index2){
					    	var img = "";
					    	...
					    	var token1 = token + "_" + toPinyin(name);
					    	token1 = removeSymbols(token1);
					    	token_2+=token1+",";
					    	chi_3 += format('<menu id="M{4}" name="{0}" token="{1}" url="{2}" level="3" img="{3}" />',name, token1, href, img, index2 + 1);
					    });
					    token_1+=token_2.substring(0,token_2.length-1);
					    chi_2 += format('<menu id="M{2}" name="{0}" token="{1}" level="2">',name, token_2.substring(0,token_2.length-1), index + 1 );
					    chi_2+=chi_3;
					    chi_2 += "</menu>";
					    chi_3="";
					    token_2="";
					...
					chi_1+=format('<menu id="{0}" name="{1}" token="{2}" level="1" url="{3}">',"M" + idx, menuArray[idx-1].name, token_1, menuArray[idx-1].href);
					token_1="";
					chi_1+=chi_2;
					chi_2="";
					chi_1 += "</menu>";
					$.post("<%= basePath%>testMenu.xhtml",{content:chi_1,idx:idx},function(data){
						menuLeft(idx, menuArray);			
					});
					chi_1 = "";
				});
			}
			...
			var removeSymbols = function(value){
				var symbols =new Array("、",",","（","）");
				
				for(var i in symbols){
					var reg=new RegExp(symbols[i],"g"); 
